  ...
  switch(inputType, 
         ...
         BAM={
           ...
           if(genome == 'tair10') {
             ...
             mapping <- mapSeqlevels(names(chrLen), "UCSC")
             if(nrow(mapping) == 0) {
               tmp <- t(matrix(chrLen))
               colnames(tmp) <- names(chrLen)
               newChrNames <- tmp
             } else {
               newChrNames <- mapping
             }
           ...
           names(chrLen) <- colnames(newChrNames)
...
GetAnnotations <- function(genome) {
  ...
  if (! is.null(mart)) {
    ...
    if(tolower(file_extension) == 'gff') {
      ...
      #txStart <- annotations_table$start
      #txEnd <- annotations_table$end
      TSS <- annotations_table$start
      TTS <- annotations_table$end
      TSS[annotations_table$strand == '-'] <- annotations_table$end[annotations_table$strand == '-']
      TTS[annotations_table$strand == '-'] <- annotations_table$start[annotations_table$strand == '-']
      
      chrName <- annotations_table[, 1]
      strand <- annotations_table[, 4]
      
      annotations <- data.frame(Transcript = rep('.', nrow(annotations_table)), 
                                Chr = chrName,
                                Strand = strand,
                                TSS = TSS,
                                TTS = TTS,
