...
plots_base_path <- '/u01/home/paula/2023/josefina/nucleosome/output/plots/sylvio'
matrix_base_path <- '/u01/home/paula/2023/josefina/nucleosome/output/plots/sylvio'
out_base_path <- '/u01/home/paula/2023/josefina/nucleosome/output/plots/sylvio'
...
main <- function() {
  ...
  for (genome in genomes) {
    ...
    matrix_file_path <- file.path(matrix_base_path,
                                  paste0('repl1/',
                                         tool, '/clean/', genome,
                                         '/2D_occ_Sites/OCC_matrix.Sites.90_180.SylJ_1_70U.RData'))
    ...
    genes_matrix_file_path <- file.path(matrix_base_path,
                                        paste0('repl1/',
                                               tool, '/clean/', genome,
                                               '/2D_occ_Sites/genes_OCC_matrix.Sites.90_180.SylJ_1_70U.RData'))
  ...
  plotFileName <- paste0(genome_id, '_figure.pdf')
...
PlotReadsLength <- function(genome, matrix_file_path, params) {
  ...
  plotsConfig$average$mainTitle <- "Sylvio"  
  
  plotsConfig$fragmentLength$xTitle <-""
  plotsConfig$fragmentLength$mainTitle <- ""
  plotsConfig$average$xTitle <- "Position relative to TAS (bp)"
  plotsConfig$average$yTitle <- "Average nucl occ"
...
PlotReadsGenes <- function(genome, genes_matrix_file_path, params) {
  ...
  occMatrix <- occMatrix[order_indexes, ]
...
GetGraphicalParams_custom <- function(simplifyPlot, squeezePlot, genesPlot=FALSE) {
  ...
  legendLabelTheme <- element_text(size = 10, angle = 0, hjust = 0.5)
    ...
    avgOccupancyTheme <- baseTheme + theme(#plot.background = element_rect(fill = "red"),
                                           ...
                                           axis.text.y.right = element_markdown(color="white", margin = margin(t = 0, r = 0, b = 0, l = 0)), 
                                           axis.title.y.right = element_blank(),
                                           axis.title.y.left = element_markdown(color="black", margin = margin(t = 0, r = 7, b = 0, l = 0)))
